...
class APIFPDB():
    ...
    #################### Month ################################    
    def get_allfpanalysis_tbl(self,startdate,enddate):
        get_allfpanalysis_tbl = """SELECT * from fpanalysis WHERE d_date >= ? AND d_date <= ?
        """
        try:
            self.cursor = self.conn.cursor()
            self.cursor.execute(get_allfpanalysis_tbl,(startdate,enddate,))
            data = self.cursor.fetchall()
            data = pd.DataFrame((tuple(t) for t in data))
            data = data.rename(columns={
                0:'ID',
            ...
            self.cursor.rollback()
            print(e)
            print('get_allfpanalysis_tbl error','(',datetime.datetime.now().strftime('%Y-%m-%d %H:%M'),')')
        else:
            self.cursor.commit()
            self.cursor.close()
            return  data    